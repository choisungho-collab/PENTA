...
-- media 버킷은 PUBLIC(읽기 공개)이지만, 업로드(INSERT/UPDATE)는 아래 정책이 있어야 anon 키로 가능.
-- ★ 함정 주의: 조건에 (select ... from storage.buckets ...) 같은 서브쿼리를 쓰면
--   그 서브쿼리가 anon 으로 실행될 때 anon 은 buckets 를 못 읽어 빈 결과 → 차단됨.
--   또 bucket_id = 'media' 리터럴도 버킷 실제 id 가 'media' 가 아니면 안 맞음.
--   → 버킷이 하나뿐이므로 조건 없이(true) 허용하는 게 가장 확실하고 안전. 삭제는 안 줌.
