...
```
penta/
├── penta_recorder.py            녹화기 (PyInstaller → Windows exe)
...
Riot 키는 **절대 클라이언트에 노출하지 않는다.** 녹화기·갤러리는 Netlify 프록시(`netlify/functions/riot.js`)를 통해서만 Riot API를 호출하고, 키는 Netlify 환경변수 `RIOT_API_KEY`에만 둔다.
